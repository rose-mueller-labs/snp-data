...
# Load data
file_path = "SNP_CSV_w_pvalues.csv"
...
# def is_evolving(p_value, threshold):
#     return float(format(float(p_value), 'f')) < (threshold)
...
while quantile <= 1.0:
    pvalues = sorted(np.array(df['PValue']))
    bottom_X_perc_index = len(pvalues) * quantile
    threshold = pvalues[math.floor(bottom_X_perc_index)]# get the p value based on ordering
    print(threshold)
    pval_th[quantile] = threshold
    df[f'Threshold{quantile}'] = df['PValue'].astype(float) < threshold
    # add a column to the df that says if the thing is evolving or not
    quantile += 0.1

with open('pvalues_and_quantiles.txt', 'w') as f:
    for quantile, pvalue in pval_th.items():
        print(f"{quantile}: {pvalue}\n", file=f)

df.to_csv("SNP_CSV_w_pvalues_with_thresholds.csv", index=False)

# with open(f'SNP_CSV_w_pvalues.csv', 'w', newline="") as file:
#     writer = csv.writer(file)
#     # writer.writerow(['Chr', 'Pos', 'Sel', 'Pop', 'Evolving', 'Freq1', 'Freq2', 'Freq3', 'Freq4'])
#     writer.writerow(['Chr', 'Pos', 'Sel', 'Pop', 'Freq1', 'Freq2', 'Freq3', 'Freq4', 'PValue'])
...
#             writer.writerow([row['chr'], row['pos'], get_sel_pop(gen)[0], get_sel_pop(gen)[1], row[POPS_DICT[gen][0]], row[POPS_DICT[gen][1]], row[POPS_DICT[gen][2]], row[POPS_DICT[gen][3]], row['pvalue']])
