...
LOC_PATH = "/media/drosophila-lab/GenomicsModels/PopSelModels/ModelPerPopSel"
...
# Save outputs
pd.DataFrame(results).to_csv(f'{thresh}_{target_pop}_{sel}pop_sel_model_results.csv', index=False)
pd.DataFrame(best_results).to_csv(f'{thresh}_{target_pop}_{sel}optimal_thresholds_pop_sel.csv', index=False)
